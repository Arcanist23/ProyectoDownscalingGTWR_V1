...
ruta_base <- getwd()
ruta_analisis <- file.path(ruta_base, "imagenes", "campo_intensidad_viento")
# Llamar a la función
graf_k_time(ws1, 6, ruta_analisis)
graf_k_time(ws3, 6, ruta_analisis)
...
ruta_base <- getwd()
ruta_analisis <- file.path(ruta_base, "imagenes", "analisis_descriptivo")
...
# Simulación ejemplo
ruta_base <- getwd()
ruta_sim <- file.path(ruta_base, "imagenes", "simulaciones")
...
ruta_base <- getwd()
ruta_train <- file.path(ruta_base, "imagenes", "entrenamiento")
...
```{r}
residuos <- base$residuos
output_dir <- file.path(ruta_train, "residuos_temporales")
ki <- 15
kj <- 16
...
```{r}
# Uso de la función
output_dir <- file.path(ruta_train, "analisis_temporal")
# Mar (y capturar métricas)
metricas_mar <- graficar_comparacion_vecindad(Xvb, ws1, pred, c(13:15), c(31:33), "mar", file.path(output_dir, "mar"))

# Valle
metricas_valle <- graficar_comparacion_vecindad(Xvb, ws1, pred, c(41:43), c(11:13), "valle", file.path(output_dir, "valle"))

# Montaña
metricas_mont <- graficar_comparacion_vecindad(Xvb, ws1, pred, c(77:79), c(20:22), "mont", file.path(output_dir, "mont"))
...
```{r}
ruta_analisis <- file.path(ruta_train, "residuos_temporales")
...
# Simulación ejemplo
ruta_pred <- file.path(ruta_base, "imagenes", "predictions")
...
```{r}
ruta_comp <- file.path(ruta_base, "imagenes", "comparacion_downscaling")
centros_i <- c(14,44,80)
centros_j <- c(32,47,23)
graficas_comparacion_campos_downscaling(ws3, ws1, Predictor$Xlb_pred, centros_i, centros_j, output_dir = ruta_comp)
```
